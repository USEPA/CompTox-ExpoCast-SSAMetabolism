...
def Commons(chunks,dfU,fragment_error,filtering):
    ...
    dft['INTENSITY0C'] = dft['old_intensity'].values / dft['max_intensity'].values * 100
    dfLt['INTENSITY0C'] = dfLt['old_intensity'].values / dfLt['max_intensity'].values * 100
...
def Score(dfL=None,dfU=None,Mass=0.0,fragment_error=0,filtering=False):
    ...
    '''
    dfi = pd.DataFrame.from_records(records,columns=['MASS','DTXCID','ENERGY','SCORE'])
    df = pd.DataFrame(columns=['MASS','DTXCID','ENERGY','SCORE'])
    '''
    if not dfi.empty:
